...
/** Older dumps were a single unnamed run; newer ones carry one phase per procedure. */
function phasesOf(dump) {
	return dump.phases ?? [{ name: "enrich", outcomes: dump.outcomes, rows: dump.rows }];
}
...
function main() {
	...
	const oldPhases = phasesOf(oldDump);
	const newPhases = phasesOf(newDump);
	const names = [...new Set([...oldPhases.map((p) => p.name), ...newPhases.map((p) => p.name)])];

	const accepted = [];
	const divergences = [];

	for (const name of names) {
		const a = oldPhases.find((p) => p.name === name);
		const b = newPhases.find((p) => p.name === name);
		if (!a || !b) {
			divergences.push({ phase: name, key: "-", kind: "-", field: "__phase", old: !!a, new: !!b });
			continue;
		}
		const oldRows = rowMap(a.rows);
		const newRows = rowMap(b.rows);
		const keys = [...new Set([...oldRows.keys(), ...newRows.keys()])].sort();
		const kinds = new Map();
		for (const r of [...a.rows, ...b.rows]) kinds.set(r.key, r.kind ?? "unknown");

		for (const k of keys) {
			const ra = oldRows.get(k);
			const rb = newRows.get(k);
			const kind = kinds.get(k) ?? "unknown";
			if (!ra || !rb) {
				divergences.push({ phase: name, key: k, kind, field: "__row", old: !!ra, new: !!rb });
				continue;
			}
			for (const f of [...new Set([...fieldsOf(ra), ...fieldsOf(rb)])]) {
				const av = read(ra, f);
				const bv = read(rb, f);
		...
		if (JSON.stringify(a.outcomes) !== JSON.stringify(b.outcomes)) {
			const rule = INTENDED.find((r) => covers(r, "*", "__outcomes"));
			(rule ? accepted : divergences).push({
				phase: name,
				key: "-",
				kind: "-",
	...
	const line = (d) =>
		`  ${String(d.phase).padEnd(10)} ${String(d.kind).padEnd(20)} ${String(d.field).padEnd(22)} ` +
		`${JSON.stringify(d.old)} -> ${JSON.stringify(d.new)}`;

	console.log(`phases: ${names.join(", ")}`);
	console.log(
		`rows/phase: ${phasesOf(oldDump)[0]?.rows.length ?? 0} (v0.9.2) vs ` +
			`${phasesOf(newDump)[0]?.rows.length ?? 0} (current)
`,
	);
