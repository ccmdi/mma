...
fn vali_generate_produces_locations() {
    ...
    let def: vali_core::MapDefinition = serde_json::from_str(definition).unwrap();
...
fn vali_generate_rejects_bad_definition() {
    let err = serde_json::from_str::<vali_core::MapDefinition>("{ not json").unwrap_err();
    assert!(!err.to_string().is_empty());
    let def: vali_core::MapDefinition =
        serde_json::from_str(r#"{ "countryCodes": ["XX"] }"#).unwrap();
