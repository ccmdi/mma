...
pub fn export_bindings() -> Result<(), String> {
    ...
    let src = std::fs::read_to_string(&out).expect("read bindings");
    let promoted: String = src
        ...
        .collect();
    std::fs::write(&out, promoted).expect("write bindings");
...
fn export_consts() -> Result<(), String> {
    let out =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../src/bindings.consts.ts");
...
"
    ...
    std::fs::write(&out, ts).map_err(|e| e.to_string())?;
