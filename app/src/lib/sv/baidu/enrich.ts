/**
 * Baidu enrichment — patches Google-semantic extra fields from sdata.
 */
...
export function registerBaiduEnrichment(): void {
	registerEnrichmentProvider({
		id: "baiduMeta",
		label: "Baidu Street View metadata",
		fieldDefs: {},
		async enrich(locations, enrichFields, ctx?: EnrichCtx) {
			const patches = new Map<number, Record<string, unknown>>();

			const targets = locations.filter((l: Location) => getLocationProvider(l) === "baidu");
			...
			for (const loc of targets) {
				...
				try {
					...
					const meta = savedId
						? await fetchBaiduMeta(savedId)
						: await resolveBaiduNear(loc.lat, loc.lng);
					if (!meta) continue;
					const full = buildBaiduExtra(meta);
					...
					log.warn("[baidu] enrich failed", loc.id, e);
				}
			}
			return patches;
		},
